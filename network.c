...
// acima disso um erro sera emitido, porem os fragmentos
// continuarão a serem calculados
...
#define COUNT_REASSEMBLE(var) ((var > 0) ? (var)-- : (var=0))
...
struct pkt_ip_fragment
{
  ...
  uint8_t c_frag;         // contttlor of fragments, limit is MAX_FRAGMENTS
  float ttl;              // lifetime of packet
};


static int is_first_frag(struct iphdr *l3, struct tcp_udp_h *l4);
static int is_frag(struct iphdr *l3);

static void insert_data_packet(struct packet *pkt,
...
static void clear_frag();

// armazena os dados da camada de transporte dos pacotes fragmentados
static struct pkt_ip_fragment pkt_ip_frag[MAX_REASSEMBLIES];
            ...
            struct sockaddr_ll *ll)
{
  ...
  // faz checagem e limpa pacotes que ainda não enviaram todos
      ...
      if (++count_reassemblies > MAX_REASSEMBLIES)
        {
          error("Maximum number of %d fragmented packets reached, "
                "packets surpluses are not calculated.", MAX_REASSEMBLIES);
          ...
          if ( pkt_ip_frag[i].pkt_id == l3->id )
            {
              ...
              if ( (ntohs(l3->frag_off) & IP_MF) == 0)
                {
                  pkt_ip_frag[i].ttl = 0;
                  COUNT_REASSEMBLE(count_reassemblies);
