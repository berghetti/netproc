...
//
// // procura o pid no array de processos, caso encontra retorna o indice do
// array static int search_pid ( const pid_t search_pid,
...
//  sendo inode coletado do arquivo '/proc/net/tcp', caso a comparação seja
//  igual, encontramos o processo que corresponde ao inode (conexão).
//
...
//       // de rede, essa indicação é habilitada para posteriormente salvar
//       dados
//       // das conexões
//       if ( exists_pid != -1 &&
...
//             {
//               // when a connection has the status TCP_TIME_WAIT the inode is
//               0
...
//       // o buffer principal struct process_t, mantendo as estatisticas de
//       rede
...
//         continue;
//
//       // conexão nova, logo não tem estatisticas a serem copiadas, copia
//       apenas
...
// // Antes de reallocar mais memoria é feita verificações para checar se o
// valor
...
// // por processos com conexões ativas, ou seja, processos que encerram e/ou
// não
...
//       if ( !count )
//         {
//           count = ENTRY_READ_NAME;
//           buff_size += ENTRY_READ_NAME;
...
//         }
//
//       bytes_read = read ( fd, p_buf, count );
//
//       if ( bytes_read == -1 )
//         {
//           if ( errno == EINTR )
