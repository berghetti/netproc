...
void exec( struct processes *processes )
{
      ...
      for (int i = 0; i < SAMPLE_SPACE_SIZE; i++)
        {
          ...
          TEST_ASSERT_EQUAL_INT ( expected_rx / SAMPLE_SPACE_SIZE, proc->net_stat.avg_Bps_rx );
          TEST_ASSERT_EQUAL_INT ( expected_tx / SAMPLE_SPACE_SIZE, proc->net_stat.avg_Bps_tx );
